...
const defaultConnectionImage = fsSync.readFileSync(path.join(__dirname, "assets", "default-menu.jpeg"));
let StickersTypes;
try {
  StickersTypes = require("wa-sticker-formatter");
} catch (e) {
  console.warn("wa-sticker-formatter not available; sticker features disabled:", e.message);
  StickersTypes = null;
}
...
const ownerNumber = ["2349021506036"];
...
async function loadSession() {
  ...
  // Accept multiple formats from the pairing server or env:
  // - vortex:~<base64(json)> (legacy)
  // - Vortex!<base64(gzip(json))...> (pairing server)
  try {
    console.log(chalk.yellow("Decoding session..."));

    // strip known prefixes (case-insensitive) and any trailing dots
    let b64 = String(sessionId).trim().replace(/^(vortex:~|vortex!)/i, "").replace(/\.+$/g, "");
    if (!b64) throw new Error("Empty session payload after prefix removal");

    const raw = Buffer.from(b64, "base64");

    // Try parsing raw as JSON first (legacy format)
    try {
      const maybeJson = raw.toString();
      const parsed = JSON.parse(maybeJson);
      fsSync.writeFileSync(credsPath, raw);
      console.log(chalk.green("Session loaded successfully (raw JSON)"));
      return parsed;
    } catch (e) {
      // Not raw JSON — try gunzip (server uses gzip)
    }

    try {
      const unzipped = zlib.gunzipSync(raw);
      const parsed = JSON.parse(unzipped.toString());
      fsSync.writeFileSync(credsPath, unzipped);
      console.log(chalk.green("Session loaded successfully (gzipped JSON)"));
      return parsed;
    } catch (e) {
      throw new Error("Failed to decode session payload: " + e.message);
    }
  } catch (e) {
...
async function connectWithPairing(malvin, useMobile) {
  ...
  let number = await question(chalk.cyan("» Enter your number (e.g., +23470467771109): "));
...
async function connectToWA() {
  ...
  malvin.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      console.log(chalk.green("[ 🤖 ] vortex Connected ✅"));
...
BotActivityFilter(malvin);	
 ...
 /// READ STATUS       
  ...
  const ownerNumbers = ["2349021506036"];
...
app.get("/", (req, res) => {
  res.redirect("/vortex.html");
});
app.listen(port, () =>
  console.log(chalk.cyan(`
╭──[ hello user ]─
│🤗 hi vortex bot is live 
╰──────────────`))
);
